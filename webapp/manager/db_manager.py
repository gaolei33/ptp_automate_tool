...
def backup(sr_number):
    ...
    cmd = 'mysqldump --no-autocommit -h %s -u %s -p%s %s | gzip > %s' % (config.DATABASE['host'], config.DATABASE['user'], config.DATABASE['passwd'], config.DATABASE['db'], backup_path)
...
def restore(backup_name):
    ...
    cmd = 'gunzip < %s | mysql -h %s -u %s -p%s %s' % (backup_path, config.DATABASE['host'], config.DATABASE['user'], config.DATABASE['passwd'], config.DATABASE['db'])
