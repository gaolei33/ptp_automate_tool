...
def bus_route_add_or_update(csv_name, method, bus_service_ids, sr_number):
    ...
    # generate SQL string
    sql = generate_sql(bus_routes_wrap_quotes, method)

    # execute the generated SQL on development database
    db_util.exec_cmds(sql)

    # save SQL string to file
    sql_name = sql_manager.get_sql_name(sr_number, method, '_'.join(bus_service_ids))
    sql_manager.save_sql(sql_name, sql)

    return sql_name


def generate_sql(bus_routes, method):
    sql = ''
    for bus_route in bus_routes:
        if method == 'BUS_ROUTE_NCS':
